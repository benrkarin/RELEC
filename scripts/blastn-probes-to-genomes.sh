...
else echo "$2 has a problem...exiting"; exit; fi
echo ""
if [ -d $3 ]
...
samtools faidx $2
cut -f1 $2.fai > query_names.txt


# Make new directories to store results
for taxon in $(cat "$4"); do # Loopthrough taxo name file 
	mkdir $3/$taxon
...
for q in $(cat query_names.txt); do
	echo "Running searches on $q"
	samtools faidx $2 $q > query_current.txt #extract current query
	for taxon in $(cat "$4"); do # Loop through taxo name file 
		blastn -query query_current.txt -db $1/$taxon.blastdb -out $3/$taxon/$q.txt -evalue 1e-10 -outfmt 6
		if [ -s $3/$taxon/$q.txt ]; then
			echo "$taxon $q Blasted at E-10"
		else
			blastn -query query_current.txt -db $1/$taxon.blastdb -out $3/$taxon/$q.txt -evalue 1e-1 -outfmt 6
			if [ -s $3/$taxon/$q.txt ]; then
